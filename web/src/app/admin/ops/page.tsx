...
export default function OpsPage() {
  ...
  const suggestions = useMemo(() => {
    ...
    if (!q || q.length < 2) return [] as Array<{ type: 'trip'|'driver', id: string, label: string }>
    const tripMatches = trips
      .filter(t => (t.id || '').toLowerCase().includes(q))
      .slice(0, 50)
      .map(t => ({ type: 'trip' as const, id: t.id, label: `Trip ${t.id}` }))
    const driverMatches = drivers
      .filter(d => (d.id || '').toLowerCase().includes(q))
      .slice(0, 50)
      .map(d => ({ type: 'driver' as const, id: d.id, label: `Driver ${d.id}` }))
    return [...tripMatches, ...driverMatches].slice(0, 50)
  }, [searchId, JSON.stringify(trips), JSON.stringify(drivers)])
  ...
  useEffect(() => {
    ...
    const qs = p.toString()
    // Cast to any to satisfy typedRoutes during programmatic replace with dynamic query
  ...
  // Keyboard shortcuts: r refresh, f toggle follow
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      const tag = (e.target as HTMLElement)?.tagName?.toLowerCase()
      if (tag === 'input' || tag === 'textarea') return
      if (e.key === 'r' || e.key === 'R') { e.preventDefault(); manualRefresh() }
      if (e.key === 'f' || e.key === 'F') { e.preventDefault(); setFollow(v => !v) }
    }
  ...
  async function manualRefresh() {
    try {
      ...
      const snaps: DriverSnap[] = (driversResp.items || []).map((d: any) => ({ id: d.userId, status: d.status, lat: d.lat ?? null, lng: d.lng ?? null, at: d.locationUpdatedAt ?? null }))
      setDrivers(snaps)
    } catch (e: any) { setError(e?.message || 'Error al refrescar') } finally { setLoading(false) }
  }
  ...
  const selectId = (type: 'trip'|'driver', id: string) => {
    setSelected({ type, id })
    setFollow(true)
  }
  ...
  function selectById() {
    try {
      ...
      if (!q) return
      // Allow prefixes to hint type
      if (q.startsWith('trp_')) {
        const t = trips.find(x => x.id === q)
        if (t) { selectId('trip', t.id); return }
      }
      if (q.startsWith('drv_')) {
        const d = drivers.find(x => x.id === q)
        if (d) { selectId('driver', d.id); return }
      }
      const t = trips.find(x => x.id === q)
      if (t) { selectId('trip', t.id); return }
      const d = drivers.find(x => x.id === q)
      ...
      <div className="card" style={{marginTop:8}}>
        <div className="row" style={{gap:8, flexWrap:'wrap', alignItems:'flex-start'}}>
          <label style={{position:'relative'}}>
            <div className="muted">Buscar ID (trip o driver)</div>
            <input
              aria-label="Buscar ID (trip o driver)"
              placeholder="trp_... o drv_..."
              list="ops-ids"
              value={searchId}
              onChange={e=>{ setSearchId(e.target.value); setShowSuggest(true) }}
              onFocus={()=>setShowSuggest(true)}
              onBlur={()=>setTimeout(()=>setShowSuggest(false), 150)}
              onKeyDown={(e)=>{ if (e.key === 'Enter') { e.preventDefault(); selectById() } }}
              style={{minWidth:280}}
            />
            <datalist id="ops-ids">
              {trips.slice(0, 200).map(t => (<option key={t.id} value={t.id}>{`Trip ${t.id}`}</option>))}
              {drivers.slice(0, 200).map(d => (<option key={d.id} value={d.id}>{`Driver ${d.id}`}</option>))}
            </datalist>
            {showSuggest && suggestions.length > 0 && (
              <ul style={{position:'absolute', top: '100%', left: 0, right: 0, zIndex: 5, background: 'var(--surface)', border: '1px solid rgba(255,255,255,0.1)', maxHeight: 220, overflowY: 'auto', listStyle: 'none', margin: 0, padding: 0}}>
                {suggestions.map(s => (
                  <li key={`${s.type}-${s.id}`}>
                    <button
                      type="button"
                      onClick={() => { setSearchId(s.id); selectId(s.type, s.id); setShowSuggest(false) }}
          ...
          <button className="btn" onClick={() => { selectById(); setShowSuggest(false) }} aria-label="Seleccionar en mapa">Seleccionar</button>
        </div>
      </div>
...
// (moved selectById logic inside component scope)
