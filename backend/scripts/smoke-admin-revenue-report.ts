// scripts/smoke-admin-revenue-report.ts
...
async function check(path: string) {
  ...
  // CSV
  let r3 = await fetch(BASE + (path.includes('?') ? path + '&' : path + '?') + 'format=csv', { headers: { authorization: 'Bearer ' + token } })
  const e2 = r3.headers.get('etag') || ''
  console.log(path, 'CSV status', r3.status, 'etag', e2)
  await r3.text()
  let r4 = await fetch(BASE + (path.includes('?') ? path + '&' : path + '?') + 'format=csv', { headers: { authorization: 'Bearer ' + token, 'if-none-match': e2 } })
  console.log(path, 'CSV If-None-Match status', r4.status)
}

async function main() {
  console.log('Smoke Admin Revenue Report @', BASE)
  await check('/admin/revenue/report?groupBy=city')
  await check('/admin/revenue/report?groupBy=method')
}
