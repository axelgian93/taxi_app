// scripts/smoke-stripe-cancel-auth.ts
...
async function main() {
  ...
  if (!stripeKey) {
    console.log('STRIPE_SECRET_KEY no definido; omitiendo smoke Stripe cancel-auth.')
    return
  }
  ...
  console.log('Created AUTHORIZED Stripe intent (to cancel):', intent.id, 'trip:', trip.id)

  // Call ADMIN refund endpoint which should cancel the authorization (status AUTHORIZED)
  const resp = await post(`/payments/${trip.id}/refund`, admin.token, { reason: 'smoke_cancel_auth' })
  console.log('Cancel auth resp:', resp)

  // Verify payment and refunds
  const payment = await get(`/payments/${trip.id}`, admin.token)
  console.log('Payment after cancel:', payment)
  const refunds = await get(`/payments/${trip.id}/refunds`, admin.token)
  console.log('Refunds:', refunds)
}

main().catch((e) => {
  console.error('Smoke stripe cancel-auth failed:', e)
  process.exit(1)
}).finally(async () => {
