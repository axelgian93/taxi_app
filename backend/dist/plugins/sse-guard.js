...
exports.default = (0, fastify_plugin_1.default)(async function sseGuard(app) {
    ...
    const g = globalThis;
    g.__sseUser = g.__sseUser || new Map();
    g.__sseIp = g.__sseIp || new Map();
    function inc(userId, ip) {
        if (userId)
            g.__sseUser.set(userId, (g.__sseUser.get(userId) || 0) + 1);
        g.__sseIp.set(ip, (g.__sseIp.get(ip) || 0) + 1);
    }
    function dec(userId, ip) {
        if (userId) {
            const v = (g.__sseUser.get(userId) || 1) - 1;
            if (v > 0)
                g.__sseUser.set(userId, v);
            else
                g.__sseUser.delete(userId);
        }
    ...
    app.addHook('onRoute', (routeOptions) => {
        ...
        if (!(method === 'GET' && /\/trips\/:id\/sse$/.test(url)))
            return;
...
//# sourceMappingURL=sse-guard.js.map