...
exports.default = adminTripsReportRoutes;
const prisma_1 = __importDefault(require("../../lib/prisma"));
const crypto_1 = __importDefault(require("crypto"));
const metrics_service_1 = require("../../services/metrics.service");
async function adminTripsReportRoutes(app) {
    const querySchema = {
        type: 'object',
        properties: {
    ...
    app.get('/admin/trips/report', {
        ...
        (0, metrics_service_1.incCounter)('admin_trips_report_queries_total');
        const { from, to, city, status, groupBy = 'city_status', format = 'json', limit = 1000 } = req.query;
        const params = [];
        const where = [];
        if (from) {
            params.push(new Date(from).toISOString());
            where.push(`t."requestedAt" >= $${params.length}`);
        }
        if (to) {
        ...
        if (city) {
            ...
            where.push(`t.city = $${params.length}`);
        }
        if (status) {
        ...
        let groupSql = '';
        if (groupBy === 'city') {
            selectSql = 't.city AS city';
            groupSql = 'GROUP BY t.city';
        }
        else if (groupBy === 'status') {
            selectSql = 't.status AS status';
            groupSql = 'GROUP BY t.status';
        }
        else {
            selectSql = 't.city AS city, t.status AS status';
            groupSql = 'GROUP BY t.city, t.status';
        }
        ...
        if (format === 'csv') {
            (0, metrics_service_1.incCounter)('admin_trips_report_csv_exports_total');
...
//# sourceMappingURL=admin.trips.report.routes.js.map