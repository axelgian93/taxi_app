...
async function adminMetricsRoutes(app) {
    app.get('/admin/metrics', { schema: { operationId: 'adminMetrics', tags: ['admin'], summary: 'Prometheus metrics', description: 'ExposiciÃ³n de mÃ©tricas en formato Prometheus. Protegido por rol ADMIN.' }, preHandler: app.auth.requireRole('ADMIN') }, async (_req, reply) => {
    ...
    app.get('/metrics', { schema: { operationId: 'metricsPublic', tags: ['admin'], summary: 'Prometheus metrics (public)', description: 'Endpoint para scraping por Prometheus. Requiere header x-metrics-token si METRICS_TOKEN estÃ¡ definido o si METRICS_PUBLIC=false.', security: [] } }, async (req, reply) => {
