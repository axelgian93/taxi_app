// src/modules/payments/payment.admin.ext.routes.ts
import type { FastifyInstance } from 'fastify'
import prisma from '../../lib/prisma'

export default async function paymentAdminExtRoutes(app: FastifyInstance) {
  const listQuery = {
    type: 'object',
    properties: {
      userId: { type: 'string' },
  ...
  app.get('/admin/payments/extended', {
    ...
    const { userId, status, method, provider, riderEmail, driverEmail, city, from, to, limit = 50, cursor, format = 'json' } = req.query as any
    const where: any = {}
    ...
    const tripWhere: any = {}
    if (userId) { tripWhere.OR = [ { riderId: userId }, { driverId: userId } ] }
    if (city) { tripWhere.pricingSnapshot = { path: ['city'], equals: city } as any }
    ...
    if (riderId) tripWhere.riderId = riderId
    if (driverId) tripWhere.driverId = driverId
    if (Object.keys(tripWhere).length > 0) where.trip = tripWhere
    ...
    const riderIds = Array.from(new Set(items.map((p: any) => p.trip?.riderId).filter(Boolean)))
    const driverIds = Array.from(new Set(items.map((p: any) => p.trip?.driverId).filter(Boolean)))
    const users = await prisma.user.findMany({ where: { id: { in: [...riderIds, ...driverIds] as any } }, select: { id: true, email: true } })
