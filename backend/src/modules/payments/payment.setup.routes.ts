...
export default async function paymentSetupRoutes(app: FastifyInstance) {
  ...
  app.post(
    ...
    { schema: { operationId: 'paymentsCreateSetupIntent', tags: ['payments'], summary: 'Crear SetupIntent', description: 'Rider crea un SetupIntent para guardar una tarjeta. Devuelve client_secret.', response: { 200: { type: 'object', properties: { clientSecret: { type: 'string' } } }, 400: { type: 'object', properties: { error: { type: 'string' } } }, 403: { type: 'object', properties: { error: { type: 'string' } }, example: { error: 'Email not verified' } }, 404: { type: 'object', properties: { error: { type: 'string' } } } } }, preHandler: app.auth.verifyJWT },
    async (req: any, reply) => {
  ...
  app.post(
    ...
    { schema: { operationId: 'paymentsSetDefaultMethod', tags: ['payments'], summary: 'Definir PM por defecto', description: 'Guarda el paymentMethod como predeterminado en Stripe y DB.', body: { type: 'object', required: ['paymentMethodId'], properties: { paymentMethodId: { type: 'string', example: 'pm_123' } } }, response: { 200: { type: 'object', properties: { ok: { type: 'boolean' } } }, 400: { type: 'object', properties: { error: { type: 'string' } } }, 403: { type: 'object', properties: { error: { type: 'string' } }, example: { error: 'Email not verified' } } } }, preHandler: app.auth.verifyJWT },
    async (req: any, reply) => {
      const userId = req.user?.id as string
