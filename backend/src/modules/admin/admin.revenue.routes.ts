// src/modules/admin/admin.revenue.routes.ts
import type { FastifyInstance } from 'fastify'
import prisma from '../../lib/prisma'
import crypto from 'crypto'

export default async function adminRevenueRoutes(app: FastifyInstance) {
  const qs = {
    type: 'object',
    properties: {
  ...
  app.get('/admin/revenue/report', {
    ...
    const { from, to, city, method, groupBy = 'city_method', format = 'json', limit = 1000 } = req.query as any
    const params: any[] = []
    const where: string[] = ['p.status = \"PAID\"']
    if (from) { params.push(new Date(from).toISOString()); where.push(`p."createdAt" >= $${params.length}`) }
    if (to) { params.push(new Date(to).toISOString()); where.push(`p."createdAt" <= $${params.length}`) }
    if (city) { params.push(String(city)); where.push(`(t.city = $${params.length} OR (t."pricingSnapshot" ->> 'city') = $${params.length})`) }
    if (method) { params.push(String(method)); where.push(`p.method = $${params.length}`) }
    const whereSql = where.length ? `WHERE ${where.join(' AND ')}` : ''

    let selectSql = ''
    let groupSql = ''
    if (groupBy === 'city') { selectSql = 'COALESCE(t.city, t."pricingSnapshot" ->> \"city\") AS city'; groupSql = 'GROUP BY 1' }
    else if (groupBy === 'method') { selectSql = 'p.method AS method'; groupSql = 'GROUP BY 1' }
    else if (groupBy === 'city_method') { selectSql = 'COALESCE(t.city, t."pricingSnapshot" ->> \"city\") AS city, p.method AS method'; groupSql = 'GROUP BY 1,2' }
    else if (groupBy === 'day') { selectSql = 'to_char(p."createdAt", \"YYYY-MM-DD\") AS day'; groupSql = 'GROUP BY 1' }
    else /* city_day */ { selectSql = 'COALESCE(t.city, t."pricingSnapshot" ->> \"city\") AS city, to_char(p."createdAt", \"YYYY-MM-DD\") AS day'; groupSql = 'GROUP BY 1,2' }
